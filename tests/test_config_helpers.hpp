...
inline void populate_minimal_slots(ChargerConfig& cfg) {
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        // Match production config topology:
        // single connector => no ring neighbors; multi-connector => simple cw/ccw ring.
        if (count <= 1) {
            sm.cw_id = 0;
            sm.ccw_id = 0;
        } else {
        ...
        // Unit tests run without real CAN/module telemetry; use the simulated module driver.
        m0.type = "sim";
        ModuleConfig m1;
